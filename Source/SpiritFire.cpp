...
void SpiritFire::GreenParticle()
{
	...
	{
		float g = IFE::OutQuad(0.6f, 0.1f, emitterPtr_->particleMaxTime_ - 0.25f, max(p->timer_ - 0.25f, 0));
		float b = IFE::Lerp(0.3f, 0, emitterPtr_->particleMaxTime_ * 3 / 4, min(p->timer_, emitterPtr_->particleMaxTime_ * 2 / 3));
		float r = IFE::OutQuad(0.1f, 0, emitterPtr_->particleMaxTime_ / 2, min(p->timer_, emitterPtr_->particleMaxTime_ / 2));
	...
	if (enemy_->GetComponent<IFE::Enemy>()->GetHP() != 0)
	{
		...
		p->GetComponent<IFE::ColorBuffer>()->SetColor({ 0.1f, 0.6f, 0.3f, 1.f });
		...
		p2->GetComponent<IFE::ColorBuffer>()->SetColor({ 0.1f, 0.6f, 0.3f, 1.f });
...
{
	&SpiritFire::BlueParticle,&SpiritFire::GreenParticle,
};