...
GraphicsPipeline* IFE::GraphicsPipelineManager::CreateShadowMapGraphicsPipeLine()
{
	string vs = defaultDirectory_ + "ShadowVS.hlsl";
	ShaderCompile(vs, SHADER_COMPILE_SETTINGS::Vertex);
	string ps = defaultDirectory_ + "ShadowPS.hlsl";
	ShaderCompile(ps, SHADER_COMPILE_SETTINGS::Pixel);
	string gs = defaultDirectory_ + "ShadowGS.hlsl";
	ShaderCompile(gs, SHADER_COMPILE_SETTINGS::Geometry);
	...
	D3D12_ROOT_PARAMETER rootParamSeed2;//5
	rootParamSeed2.ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;		//種類
	rootParamSeed2.DescriptorTable.pDescriptorRanges = &TextureManager::Instance()->GetDescRangeSRV();				//デスクリプタレンジ
	rootParamSeed2.DescriptorTable.NumDescriptorRanges = 1;							//デスクリプタレンジ数
	rootParamSeed2.ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;					//すべてのシェーダーから見える
	...
	D3D12_ROOT_PARAMETER rootParamSeed1;//6
	//定数用
	rootParamSeed1.ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;		//種類
	CD3DX12_DESCRIPTOR_RANGE descRangeSRV1;
	descRangeSRV1.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 1); // t1 レジスタ
	rootParamSeed1.DescriptorTable.pDescriptorRanges = &descRangeSRV1;				//デスクリプタレンジ
	rootParamSeed1.DescriptorTable.NumDescriptorRanges = 1;							//デスクリプタレンジ数
	rootParamSeed1.ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;					//すべてのシェーダーから見える
	rootParams.push_back(rootParamSeed1);

	D3D12_ROOT_PARAMETER rootParamSeed;//7
	//定数用
	rootParamSeed.ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;				//種類
	...
	if (CreateGraphicsPipeline("3dShadow", rootSignatureDesc, pipelineDesc, (uint8_t)PIPELINE_SETTING::Shadow))
	{
		return nullptr;
	}
	return pipelineList_.back().get();
...
GraphicsPipeline* IFE::GraphicsPipelineManager::CreateDepthWriteGraphicsPipeLine()
{
	string vs = defaultDirectory_ + "DepthWriteVS.hlsl";
	ShaderCompile(vs, SHADER_COMPILE_SETTINGS::Vertex);
	string ps = defaultDirectory_ + "DepthWritePS.hlsl";
	ShaderCompile(ps, SHADER_COMPILE_SETTINGS::Pixel);
	...
	if (CreateGraphicsPipeline("3dDepthWrite", rootSignatureDesc, pipelineDesc, (uint8_t)PIPELINE_SETTING::DepthWrite))
	{
		return nullptr;
	}
	return pipelineList_.back().get();
